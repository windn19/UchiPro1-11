import sys

from PyQt5.QtCore import Qt
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QPushButton, QLCDNumber


class MainWindow(QWidget):
    """Класс окна"""
    def __init__(self):
        """Инициализация элементов"""
        # задание основных параметров окна
        super().__init__()
        self.move(700, 300)
        self.resize(300, 200)
        self.setWindowTitle("Кликер")

        # создание элементов
        button_1 = QPushButton("Нажми меня", self)
        button_2 = QPushButton("Сбросить", self)
        self.lcd = QLCDNumber(self)

        # создание рамки и добавление в нее элементов
        vbox = QVBoxLayout()
        vbox.addWidget(self.lcd)
        vbox.addWidget(button_1)
        vbox.addWidget(button_2)

        # добавление рамки в основное окно
        self.setLayout(vbox)

        # соединение сигнала "нажатие на кнопку" со слотом self.button_was_clicked у первой и второй кнопки
        button_1.clicked.connect(self.button_was_clicked)
        button_2.clicked.connect(self.button_was_clicked)

        # создание кнопки счетчика
        self.counter = 0

    def button_was_clicked(self):
        """Обработка нажатия на кнопку"""
        sender = self.sender()  # определение объекта, который вызвал сигнал
        # проверка текста кнопки
        if sender.text() == 'Нажми меня':
            self.counter += 1  # добавление счетчика
        elif sender.text() == 'Сбросить':
            self.counter = 0  # сброс счетчика
        self.lcd.display(self.counter)  # вывод на экран значения счетчика

    def keyPressEvent(self, event):
        """Реакция на нажатие кнопки клавиатуры"""
        if event.key() == Qt.Key_Escape:  # нажата кнопка Escape
            self.close()  # закрытие окна
        elif event.key() == Qt.Key_1:  # нажата кнопка 1
            self.counter += 1  # увеличение счетчика
        elif event.key() == Qt.Key_2:  # нажата кнопка 2
            self.counter -= 1  # уменьшение счетчика
        self.lcd.display(self.counter)  # вывод значения счетчика


if __name__ == '__main__':
    app = QApplication(sys.argv)  # создание приложения
    window = MainWindow()  # создание объекта класса нашего окна
    window.show()  # вывод на экран
    app.exec()
